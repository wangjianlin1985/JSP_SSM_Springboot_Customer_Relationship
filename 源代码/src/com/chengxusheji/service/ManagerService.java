...
public class ManagerService {
    ...
    /*按照查询条件分页查询管理员记录*/
    public ArrayList<Manager> queryManager(String managerUserName,String name,Department departmentObj,int currentPage) throws Exception { 
    	...
    	if(null != departmentObj &&  departmentObj.getDepartmentNo() != null  && !departmentObj.getDepartmentNo().equals(""))  where += " and t_manager.departmentObj='" + departmentObj.getDepartmentNo() + "'";
    	int startIndex = (currentPage-1) * this.rows;
    	return managerMapper.queryManager(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<Manager> queryManager(String managerUserName,String name,Department departmentObj) throws Exception  { 
     	String where = "where 1=1";
    	if(!managerUserName.equals("")) where = where + " and t_manager.managerUserName like '%" + managerUserName + "%'";
    	if(!name.equals("")) where = where + " and t_manager.name like '%" + name + "%'";
    	if(null != departmentObj &&  departmentObj.getDepartmentNo() != null && !departmentObj.getDepartmentNo().equals(""))  where += " and t_manager.departmentObj='" + departmentObj.getDepartmentNo() + "'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String managerUserName,String name,Department departmentObj) throws Exception {
     	String where = "where 1=1";
    	if(!managerUserName.equals("")) where = where + " and t_manager.managerUserName like '%" + managerUserName + "%'";
    	if(!name.equals("")) where = where + " and t_manager.name like '%" + name + "%'";
    	if(null != departmentObj &&  departmentObj.getDepartmentNo() != null && !departmentObj.getDepartmentNo().equals(""))  where += " and t_manager.departmentObj='" + departmentObj.getDepartmentNo() + "'";
