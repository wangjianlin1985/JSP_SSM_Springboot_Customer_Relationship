...
public class ManagerController extends BaseController {
	...
	public void list(String managerUserName,String name,@ModelAttribute("departmentObj") Department departmentObj,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (managerUserName == null) managerUserName = "";
		if (name == null) name = "";
		if(rows != 0)managerService.setRows(rows);
		List<Manager> managerList = managerService.queryManager(managerUserName, name, departmentObj, page);
	    /*计算总的页数和总的记录数*/
	    managerService.queryTotalPageAndRecordNumber(managerUserName, name, departmentObj);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String managerUserName,String name,@ModelAttribute("departmentObj") Department departmentObj,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (managerUserName == null) managerUserName = "";
		if (name == null) name = "";
		List<Manager> managerList = managerService.queryManager(managerUserName, name, departmentObj, currentPage);
	    /*计算总的页数和总的记录数*/
	    managerService.queryTotalPageAndRecordNumber(managerUserName, name, departmentObj);
	    /*获取到总的页码数目*/
	    int totalPage = managerService.getTotalPage();
	...
	public void OutToExcel(String managerUserName,String name,@ModelAttribute("departmentObj") Department departmentObj, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(managerUserName == null) managerUserName = "";
        if(name == null) name = "";
        List<Manager> managerList = managerService.queryManager(managerUserName,name,departmentObj);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Manager信息记录"; 
        String[] headers = { "管理用户名","姓名","性别","年龄","所在部门"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<managerList.size();i++) {
        	Manager manager = managerList.get(i); 
        	dataset.add(new String[]{manager.getManagerUserName(),manager.getName(),manager.getSex(),manager.getAge() + "",manager.getDepartmentObj().getDepartmentName()});
        }
        /*
